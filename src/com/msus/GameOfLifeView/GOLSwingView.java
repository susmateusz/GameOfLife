...
public class GOLSwingView extends JFrame implements View {
	...
	private List<Coords> active = new ArrayList<Coords>();
	...
	public GOLSwingView(int n, int m) {
		...
		btnLoadState.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (String line : coordsList.getText().split("\n")) {
					...
					Coords coords = new Coords(Integer.parseInt(line.split(" ")[0].trim()),Integer.parseInt(line.split(" ")[1].trim()));
					model.setCellState(coords, State.ALIVE);
				}
				model.hasChanged();
				model.notifyObservers(model.toArrayOfState(State.ALIVE));
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				Coords currentCoords = new Coords((e.getY() - 30) / CELL_SIZE,(e.getX() - 8) / CELL_SIZE);
				if (currentCoords.getX() < bounds.get(0) && currentCoords.getY() < bounds.get(1)) {
		...
		addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				...
				Coords currentCoords = new Coords((e.getY() - 30) / CELL_SIZE,(e.getX() - 8) / CELL_SIZE);
				if (currentCoords.getX() < bounds.get(0) && currentCoords.getY() < bounds.get(1)) {
					labelPos.setText("Cursor pos: (" + currentCoords.getX() + ", " + currentCoords.getY() + ")");
					Set<Coords> mouseShadowPoints = ((GOLPattern) selectPattern.getSelectedItem())
